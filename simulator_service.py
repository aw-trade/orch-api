...
class SimulatorService:
    ...
    def start_simulation(self, run_id: str, duration_seconds: int, algo_consts=None, simulator_consts=None) -> bool:
        # Check if containers are actually running first
        self._sync_status_with_docker()
    ...
    def _sync_status_with_docker(self):
        """Sync internal status with actual Docker container state"""
        try:
            # Check if any simulation containers are running
            result = subprocess.run(['docker', 'compose', 'ps', '-q'], 
                                  capture_output=True, text=True, cwd='.')
            
            # If no containers and we think we're running, reset to idle
            if result.returncode == 0 and not result.stdout.strip():
                if self.status in [SimulationStatus.RUNNING, SimulationStatus.STARTING, SimulationStatus.ERROR]:
                    logger.info("No Docker containers found, resetting status to IDLE")
                    self.status = SimulationStatus.IDLE
                    self.start_time = None
                    self.duration = None
                    self.current_run_id = None
            ...
            logger.warning(f"Failed to sync Docker status: {e}")
    
    def get_status(self) -> dict:
        # Always sync with Docker state before returning status
        self._sync_status_with_docker()
